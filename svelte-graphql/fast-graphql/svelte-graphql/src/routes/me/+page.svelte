<script lang="ts">
  ...
  let id = $derived(profile.id);
  let email = $derived(profile.email);
  let name = $derived(profile.name);
  let fullname = $derived(profile?.fullname);
  let rol = $derived(profile.rol);
  let secondName = $derived(profile?.secondName);
  let streetAdress = $derived(profile?.streetAdress);
  let city = $derived(profile?.city);
  let province = $derived(profile?.province);
  let zipCode = $derived(profile?.zipCode);
...
<!-- USER PROFILE -->
<form class="space-y-8 container m-auto divide-y divide-gray-200 mt-3">
      ...
      <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
        ...
        <div class="sm:col-span-4">
          ...
          <div class="mt-1 flex rounded-md shadow-sm">
            ...
            <input bind:value={name} type="text" name="username" id="username" autocomplete="username" class="flex-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full min-w-0 rounded-none rounded-r-md sm:text-sm border-gray-300">
          </div>
        </div>

        <!-- SOBRE MI -->
        <div class="sm:col-span-6">
          <label for="about" class="block text-sm font-medium text-gray-700"> Sobre mí </label>
          <div class="mt-1">
          ...
          <p class="mt-2 text-sm text-gray-500">Escribe un par de lineas sobre ti</p>
        </div>
        ...
        <div class="sm:col-span-6">
          ...
          <div class="mt-1 flex items-center">
            ...
            <button type="button" class="ml-5 bg-white py-2 px-3 border border-gray-300 rounded-md shadow-sm text-sm leading-4 font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Cambiar</button>
          </div>
        </div>

        <!-- Cover Foto -->
        <div class="sm:col-span-6">
          <label for="cover-photo" class="block text-sm font-medium text-gray-700"> Cover foto </label>
          <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
            <div class="space-y-1 text-center">
              ...
              <div class="flex text-sm text-gray-600">
                ...
                <p class="pl-1">o arrastra y sueltea</p>
              </div>
