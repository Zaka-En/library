<script lang="ts">
  ...
  const categories: CategoryType[] = [
    {
      ...
      description:
        "Viajes en el tiempo, distopías futuristas y exploración espacial.",
      totalBooks: 124,
    },
    {
      name: "Filosofía",
      description:
        "Tratados clásicos y pensamientos contemporáneos sobre la existencia.",
      totalBooks: 86,
    },
    {
      name: "Desarrollo Web",
      description:
        "Guías modernas sobre Svelte, Tailwind y arquitectura de software.",
      totalBooks: 52,
    },
  ];
  ...
  const myReadingProgress = $derived(
    $myReadingProgressStore.data?.myReadingProgress || [],
  );
  console.log("==========================================================");
...
{:else if $myReadingProgressStore.errors}
  <div class="bg-red-100 text-red-700 p-4 rounded">
    Ouups: ha habido un error al obtener sus datos. Contacta con el equipo de
    soporte
  </div>
{/if}
...
{#if myReadingProgress && !$myReadingProgressStore.fetching}
  <div>
    ...
    <section
      class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-9/12"
    >
...
{#snippet categorySnippet(
  ...
  <a
    href="/books"
    class="px-3 py-1 bg-indigo-600 text-white text-xs font-semibold rounded-full hover:bg-indigo-700 transition-colors"
  >
...
<Categories {categories} {categorySnippet}>
  {#snippet children(
    cat: CategoryType,
    renderSnippet: Snippet<[string, string, number]>,
  )}
