...
from app.schema.inputs import LoginInput, RegisterInput, UpdateUserInput
...
class UserService:
    ...
    async def update(self, data: UpdateUserInput) -> User:
        async with self.session_factory() as session:
            update_data = {
                k: v for k, v in vars(data).items() if v is not None and k != "id"
            }

            if update_data:
                await session.execute(
                    update(User).where(User.id == data.id).values(**update_data)
                )
                await session.commit()
