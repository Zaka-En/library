...
class BookService(SingletonService, BaseService[Book]):
    ...
    async def get_by_ids(self, books_ids: list[int]) -> List[Book]:
        async with self.session_factory() as session:
            return list(
                (await session.execute(select(Book).where(Author.id.in_(books_ids))))
                .scalars()
                .all()
            )
            ...
            return list(
                (
                    await session.execute(
                        select(Book).where(Book.author_id.in_(authors_ids))
                    )
                )
                .scalars()
                .all()
    ...
    async def create(self, data: CreateBookInput):
        async with self.session_factory() as session:
            author_check = await session.execute(
                select(Author).filter(Author.id == data.author_id)
            )
            if not author_check.scalar_one_or_none():
                raise ValueError(
                    f"No se puede crear el libro: El autor {data.author_id} no existe"
                )
            IsbnQuery = await session.execute(
                select(Book).filter(Book.isbn == data.isbn)
            )
