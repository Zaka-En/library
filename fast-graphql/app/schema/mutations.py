...
from app.services.room_booking_service import IntegrityError, RoomBookingService
...
class Mutation:
    ...
    async def register_user(
        self, info: Info[CustomContext, None], data: RegisterInput
    ) -> UserType:
    ...
    @strawberry.mutation
    async def login(
        self, info: Info[CustomContext, None], data: LoginInput
    ) -> LoginResponse:
    ...
    async def update_user(
        self, info: Info[CustomContext, None], input: UpdateUserInput
    ...
    async def create_author(
        self, input: CreateAuthorInput, info: Info[CustomContext, None]
    ) -> AuthorType:
    ...
    async def update_author(
        self, input: UpdateAuthorInput, info: Info[CustomContext, None]
    ) -> AuthorType:
        author_service: AuthorService = info.context.author_service
    ...
    async def create_book(
        self, input: CreateBookInput, info: Info[CustomContext, None]
    ) -> BookType:
    ...
    async def update_book(
        self, input: UpdateBookInput, info: Info[CustomContext, None]
    ) -> BookType:
        book_service = info.context.book_service
    ...
    async def start_reading(
        self, input: StartReadingInput, info: Info[CustomContext, None]
    ) -> ReadingStateType:
        service: ReadingStateService = info.context.reading_state_service
        reading_state = await service.start_reading(
            book_id=input.book_id, user_id=input.user_id
        )
    ...
    async def update_progress(
        self, input: UpdateProgressInput, info: Info[CustomContext, None]
    ) -> ReadingStateType:
        service: ReadingStateService = info.context.reading_state_service
        return reading_state_to_type(
            await service.update_progress(
                state_id=input.id, current_page=input.current_page
            )
        )

    @strawberry.mutation  # (permission_classes=[IsAuthenticated])
    async def finish_reading(
        self, input: FinishReadingInput, info: Info[CustomContext, None]
    ) -> ReadingStateType:
        service: ReadingStateService = info.context.reading_state_service
    ...
    async def book_conference_room(
        self, input: RoomBookingInput, info: Info[CustomContext, None]
    ) -> RoomBookingType:
    ...
    async def send_book_chat_message(
        self, book_id: int, user_name: str, message: str
    ) -> bool:
