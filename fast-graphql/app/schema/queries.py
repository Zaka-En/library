...
class Query:
    ...
    async def user_info(
        self, info: Info[CustomContext, None], user_id: int
    ) -> UserProfileType:
    ...
    @strawberry.field
    async def conference_rooms(
        self, info: Info[CustomContext, None]
    ) -> List[ConferenceRoomType]:
        conference_room_service: ConferenceRoomService = (
            info.context.conference_room_service
        )
    ...
    async def authors(
        ...
        result = await author_service.get_paginated(
            limit=first or 5, after_cursor=after
        )
    ...
    @strawberry.field
    async def author(
        self, id: int, info: Info[CustomContext, None]
    ) -> Optional[AuthorType]:
        author_service: AuthorService = info.context.author_service
    ...
    @strawberry.field
    async def my_reading_progress(
        self, user_id: int, info: Info[CustomContext, None]
    ) -> List[ReadingStateType]:
    ...
    async def recomendations(self, user: str) -> Tuple[str, str, str]:
        ...
        results = await asyncio.gather(
            recomender("Drama"), recomender("Horror"), recomender("Sitcom")
        )
